...
export const getKawasanList = (): string[] => {
  const kawasanEnv = process.env.KAWASAN
  
  if (!kawasanEnv) {
    throw new Error('KAWASAN environment variable is required')
  }
  
  try {
    const parsed = JSON.parse(kawasanEnv)
    if (!Array.isArray(parsed)) {
      throw new Error('KAWASAN must be a JSON array')
    }
    if (parsed.length === 0) {
      throw new Error('KAWASAN array cannot be empty')
    }
    return parsed
  } catch (error) {
    if (error instanceof SyntaxError) {
      throw new Error(`Failed to parse KAWASAN environment variable: ${error.message}`)
    }
    throw error
  }
}
